...
using the ai_backend abstraction layer. Works with claude CLI, Anthropic API,
OpenAI API, or any available backend.
"""
...
from scripts.ai_backend import call_ai_text, detect_backend
...
def improve_description(
    ...
    """Call AI to improve the description based on eval results."""
    failed_triggers = [
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = call_ai_text(prompt, model, backend=backend)
    ...
    if len(description) > 1024:
        ...
        shorten_text = call_ai_text(shorten_prompt, model, backend=backend)
        match = re.search(
...
def main():
    ...
    parser.add_argument(
        "--backend", default=None, help="AI backend: claude, anthropic, openai"
    )
    parser.add_argument(
